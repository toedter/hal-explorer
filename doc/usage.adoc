...
Just below the URI you find the top-level JSON properties. If the resonse contains links, you find them below.
...
When you click on `Go!`, a HTTP call to the `Expanded URI` will be made.

== Embedded Resources

If the response contains embedded resources, you find the in a section below the links.
...
Whenever you click on a HTTP POST button, a modal dialog pops up, where you can enter body of the post, and the query parameters (only if the URI is templated). In raw HAL based documents, it looks like
...
When HAL-FORMS is used, the field are also validated and validation errors are displayed below the input area.
Only for valid forms the body is displayed, and the HTTP request can mit made.
...
An simple example JSON looks like

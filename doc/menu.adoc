...
HAL Explorer provides a rich theming experience with support for all https://bootswatch.com/[Bootswatch] themes (by Thomas Park).
The default theme is *Cosmo*, which offers a clean, modern look that works well with both light and dark color modes.

You can select from 25+ professionally designed themes to customize the look and feel of HAL Explorer:

* *Default* - HAL Explorer's custom default theme
* *Cosmo* - Modern, flat design with blue accents (default)
...
The _Settings_ submenu allows you to customize HAL Explorer's behavior and user interface. All settings are automatically saved to your browser's local storage with the `hal-explorer.` prefix and persist across sessions.
